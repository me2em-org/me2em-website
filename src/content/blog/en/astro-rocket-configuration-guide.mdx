...
description: "A walkthrough of every configuration option: 8 colour themes, OKLCH colours, typography, radius and shadow tokens, header styles, and dark mode."
...
Astro Rocket ships with **8 colour themes**. All 8 are shown as coloured swatches in the header via the `ThemeSelector` component. Clicking a swatch switches the active theme instantly — no file edits, no rebuild. The logo badge, blog image gradients, and every brand colour on the page update live together.
...
Available values — all 8 themes:

| Value | Hue | Best for |
|-------|-----|----------|
| `purple` | 296° | Expressive, personality-forward, vivid |
| `violet` | 279° | Creative and premium — sweet spot between indigo and purple |
| `indigo` | 263° | Enterprise, productivity tools, serious B2B |
| `blue` | 255° | Studio aesthetic (Linear/Raycast feel), authoritative — the default |
| `sky` | 239° | Clean and airy — between cyan and blue, great for services |
| `cyan` | 208° | Fresh, vibrant tech — more energetic than teal |
| `teal` | 190° | Developer tooling, modern SaaS, trustworthy services |
| `emerald` | 151° | All-rounder — works for SaaS, portfolios, and organic brands |

### The live theme selector

All 8 themes appear as colour swatches in the header dropdown (desktop) and in the mobile menu when `showThemeSelector` is enabled on the `<Header>` component. The visitor's choice is saved to `sessionStorage` and persists for the duration of the tab session — every new tab resets to the configured default. This is intentional for a portfolio or marketing site: first impressions stay on-brand. (The colour mode — System / Light / Dark — is stored separately in `localStorage` and *does* persist across reloads and new tabs; see [the colour-mode post](/blog/colour-mode-system) for that side of the system.)

The `colourThemes` array in `src/lib/themes.ts` is the single source of truth — it controls which swatches are shown and in what order, and both the desktop row and the dropdown read it. The current set:
...
Set `showInSelector: false` to hide a theme from the pickers while keeping its palette working — useful when a visitor has one saved from an earlier session.
...
html[data-theme="teal"] {
  ...
  --brand-300: oklch(78.5% 0.136 190);
  --brand-400: oklch(72.0% 0.125 189);
  --brand-500: oklch(67.2% 0.116 190); /* primary accent in light mode */
  --brand-600: oklch(57.4% 0.099 189);
  --brand-700: oklch(48.9% 0.085 189);
  --brand-800: oklch(39.9% 0.069 189);
  --brand-900: oklch(28.3% 0.049 190);
}
```

To shift to a different colour, replace `190` (teal) with your target hue across all ten lines. That's the entire change required.
...
```css
:root {
  --brand-500: oklch(62.1% 0.207 255); /* primary brand color */
...
All 8 themes currently share the same font stack. To use a custom font, install it in `public/fonts/`, add a `@font-face` declaration in `src/styles/global.css`, and update `--theme-font-sans` or `--theme-font-display` in your active theme file.
...
| Live theme selector (8 swatches) | `src/lib/themes.ts` | `colourThemes` array |
| All 8 theme files | `src/styles/themes/` | One CSS file per theme |
