...
**Header** — fixed top navigation bar with logo, nav links, the colour-mode pill (System / Light / Dark dropdown), the colour-theme pill (8-swatch picker), and an optional CTA button. Three rendering variants: `solid`, `transparent`, and `default`. The capsule header on the homepage is a separate `shape="floating"` prop — distinct from the variant. Includes the scroll progress bar when enabled.
