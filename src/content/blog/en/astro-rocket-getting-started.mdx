...
Astro Rocket ships with 8 ready-to-use colour themes. Pick your preferred theme by setting the `data-theme` attribute in `src/layouts/BaseLayout.astro`:

```html
<html lang="en" class="scroll-smooth dark" data-theme="blue">
```

Available values: `emerald`, `teal`, `cyan`, `sky`, `blue`, `indigo`, `violet`, `purple`. The theme CSS files live in `src/styles/themes/` — open the active one to fine-tune the `--brand-*` colour scale if needed.
